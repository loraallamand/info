...
plt.imshow(tab*255)
tab[:,:,:]=0
tab2=255*np.ones(shape=(91, 91, 3), dtype=np.uint8)
tab2[:,:,2:3]=0
plt.imshow(tab2)
tab3=255*np.ones(shape=(91, 91, 3), dtype=np.uint8)
tab3[: :10, : :, 0:2:]=0
tab3[: :,: :10, 0:2]=0
plt.imshow(tab3) 
...
from os import *
print(getcwd())
t = plt.imread('les-mines-sophia.jpg')
plt.imshow(t)
...
# %%
# votre code
tab = plt.imread('les-mines-sophia.jpg')
tab2 = tab[: :2, : :2, : :]
plt.imshow(tab2)
...
# %%
def image(l, c):
    t3 = tab[78-l//2:78+l//2 :, 160-c//2:160+c//2+1, : :]
    return plt.imshow(t3)
image(100,200)
...
Tg=T[: :, : :, 1:2]
Tb=T[: :, : :, 2:3]
plt.imshow(Tr, cmap ='Reds')

# %% {"scrolled": true}
plt.imshow(Tg, cmap='Greens')

# %% {"scrolled": false}
plt.imshow(Tb, cmap='Blues')

# %%
T2=T.copy()
T2[157-100: :, 321-100: :, : :]=(219, 112, 147)
plt.imshow(T2)

# %%
T2[157-100: :, 321-100: :, : :]=(255, 255, 255)
T2[157-100: :10, 321-100: :, 0:1 :]=255
plt.imshow(T2)
...
# %%
# votre code
Tt = plt.imread('les-mines-sophia.jpg')
alpha = np.empty(shape =(157, 321, 4), dtype=np.uint8)
alpha[: :, : :, 0:3 :]=Tt
alpha[: :, : :, 3:4 :]=128
plt.imshow(alpha)
alpha.dtype
...
print(type(axes))
print(axes.shape)
axes[0,0].imshow(N, cmap='Greys')
axes[1,1].imshow(N, cmap='Greys')
axes[2,2].imshow(N, cmap='Greys')
axes[0,1].imshow(Nb, cmap='Greys')
axes[1,2].imshow(Nb, cmap='Greys')
axes[2,1].imshow(Nb, cmap='Greys')
axes[0,2].imshow(K, cmap='Greys')
axes[1,0].imshow(K, cmap='Greys')
axes[2,1].imshow(K, cmap='Greys')
